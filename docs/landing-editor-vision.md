...
- Les **images** : après le texte / les slots `media`, l’API envoie vers **S3** (Imagine pour chaque slot mappé, hydratation pour les `imageUrl` déjà présentes, upload direct pour les fichiers utilisateur) ; les références vivent dans le document de version.
...
| Imagine → **S3** (tous les slots `media` mappés, hero inclus ; repli `imagePrompts.hero`) + URL signée + `imageHistory` (`section:slot`) | Fait — `POST …/generate-all-imagine-s3` |
| **Hydratation** : `imageUrl` déjà dans le JSON (placeholders `/ai/…`, `/images/…`, http) → S3 | Fait — `POST …/hydrate-image-urls-s3` |
| **Prévisualisation** Mongo (rendu `DeckLandingView` comme `/deck/:slug`) | Fait — `/admin/landing-editor/:projectId/preview/:versionId` |
| Liste **slots image** + upload / scène / alt + brief visuel | Fait — UI ; `PATCH …/image-slot` ; `PATCH …/content-globals` (`visualBrief`) |
| **Variantes de prompt** Grok → `promptAlternativesEn` + appliquer comme scène | Fait — `POST …/suggest-prompt-alternatives` + UI slot |
| **Export prompt** Imagine (EN) + préférence `primaryModel` (Grok / MJ / aucun) | Fait — `GET …/assembled-image-prompt` + `PATCH …/image-slot` |
| Éditeur **split** (aperçu `DeckLandingView` + panneau) + modes split / panneau seul / aperçu seul | Fait — fiche projet |
| Fonds **page** (`globals.backgroundImage`) + **section** (`section.backgroundImage`) dans le rendu | Fait — `DeckLandingView` + sectionRegistry ; édition URL fond page dans l’UI |
| Mode visiteur « strict » sans chrome (hors route dédiée) | À faire |
...
- Imagine → S3 (toutes sections, hero comme tout autre slot) : `POST …/generate-all-imagine-s3` — slots `media` avec `sceneDescription`, mapping connu (hero bannière, `creator`, témoignage, `photo_gallery` + `photo-N` / `item-N`, ou `slotId === sectionId`) ; si le hero n’a pas de `sceneDescription` utilisable, repli sur `imagePrompts.hero`.
- Hydratation URLs → S3 : `POST …/hydrate-image-urls-s3` — parcourt récursivement les clés `imageUrl` du `content`.
- Slot image : `PATCH …/image-slot` — `{ sectionId, slotId, resolved?, sceneDescription?, imageAlt?, primaryModel? }` ; après `POST …/assets`, `resolved: { imageUrl, source: 'upload' }`.
- Globals : `PATCH …/content-globals` — `visualBrief`, `visualBriefMarkdown`, `backgroundImage: { imageUrl, imageAlt? }`, `clearBackgroundImage`.
- Variantes de prompt image : `POST …/suggest-prompt-alternatives` — `{ sectionId, slotId, count? }` (5–12, défaut 6) → Grok → `generation.promptAlternativesEn`.
- Prompt assemblé (copie Midjourney / autre) : `GET …/assembled-image-prompt?sectionId=&slotId=` ; préférence modèle : `PATCH …/image-slot` avec `primaryModel`: `grok_imagine` \| `midjourney` \| `none`.
- Upload utilisateur : `POST …/assets` (déjà S3). La fusion Mongo conserve `imageHistory` si le document fusionné le définit ; sinon garde l’existant.
