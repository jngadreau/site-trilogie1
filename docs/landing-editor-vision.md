...
| Imagine → **S3** (tous les slots `media` mappés) + URL signée + `imageHistory` (`section:slot`) | Fait — `POST …/generate-all-imagine-s3` ; hero seul : `…/generate-hero-s3` |
...
- Imagine → S3 (toutes sections) : `POST …/generate-all-imagine-s3` — slots `media` avec `sceneDescription`, mapping connu (hero bannière, `creator`, témoignage, `photo_gallery` + `photo-N` / `item-N`, ou `slotId === sectionId`).
- Hero seul : `POST …/generate-hero-s3` (repli `imagePrompts.hero`).
