...
| Étape | Statut |
|-------|--------|
| Schémas Mongoose + API CRUD projets / versions | En cours |
| Service S3 (upload, URL signée, préfixe `cvapp/…`) | En cours |
| `/admin` allégée + route legacy deck landing | En cours |
| Wizard UI + Grok structure souple | À faire |
| Grok remplissage section par section + upload images S3 | À faire |
