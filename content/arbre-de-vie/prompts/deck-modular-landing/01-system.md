...
Produire **un seul objet JSON valide** (aucun texte hors JSON) qui décrit une landing **modulaire** : paramètres visuels **globaux** cohérents + **exactement 12 sections** dans l’ordre imposé, chacune avec le **variant** demandé et des **props** complètes.
...
- **Photos ambiance** : pour **`PhotoSpotlightGrid`** et **`PhotoFilmstripRow`**, `imageUrl` = chemins **réels** sous `/ai/generated-images/…` (bannière, coffret, lifestyle si disponibles ; sinon réutiliser la bannière hero ou des cartes en gros plan). Noms d’`alt` en français.
...
- Ordre des sections dans le tableau `sections` : **1) hero**, **2) deck_identity**, **3) for_who**, **4) outcomes**, **5) how_to_use**, **6) in_the_box**, **7) card_gallery**, **8) photo_gallery**, **9) faq**, **10) creator**, **11) related_decks**, **12) cta_band** (champs `id` exacts).
