...
Produire **un seul objet JSON valide** (aucun texte hors JSON) qui décrit une landing **modulaire** : paramètres visuels **globaux** cohérents + **exactement 10 sections** dans l’ordre imposé, chacune avec le **variant** demandé et des **props** complètes.
...
- **Cartes du jeu** : pour `HeroCardsFan`, `HeroCardsStrip`, `HeroCardsMosaic`, remplis `cards[].imageUrl` avec **`/ai/generated-images/deck-cards/<nom-fichier>`** en reprenant **exactement** un nom renvoyé par `GET /ai/generated-images/deck-cards` (ex. `card_17_front.png`) — **ne pas inventer** de noms type `oracle-…png`. Après `POST /site/sync-deck-card-images` les fichiers viennent de `images-jeux/`. **`media`** du hero : **`[]`**. Noms d’`alt` en français.
...
- Ordre des sections dans le tableau `sections` : **1) hero**, **2) deck_identity**, **3) for_who**, **4) outcomes**, **5) how_to_use**, **6) in_the_box**, **7) faq**, **8) creator**, **9) related_decks**, **10) cta_band** (champs `id` exacts).
