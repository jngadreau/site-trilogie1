...
- **Cartes du jeu** : pour `HeroCardsFan`, `HeroCardsStrip`, `HeroCardsMosaic`, remplis `cards[].imageUrl` avec **`/ai/generated-images/deck-cards/<nom-fichier>`** (après `POST /site/sync-deck-card-images` qui copie depuis `images-jeux/` ; liste : `GET /ai/generated-images/deck-cards`). **`media`** du hero : **`[]`**. Noms d’`alt` en français.
