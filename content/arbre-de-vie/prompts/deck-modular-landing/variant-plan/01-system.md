...
Produire **un seul objet JSON** (aucun texte hors JSON) qui décrit une **combinaison de variantes** pour une landing : pour chaque **type de section** (`hero`, `deck_identity`, `for_who`, `outcomes`, `how_to_use`, `in_the_box`, `card_gallery`, `photo_gallery`, `faq`, `creator`, `testimonials`, `newsletter_cta`, `related_decks`, `cta_band`), choisir **exactement un** nom de composant React parmi ceux documentés dans les fichiers Markdown fournis.
